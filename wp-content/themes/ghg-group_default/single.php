 ...
 *
 * @package HUNTER_GEE_HOLROYD_THEME
 */
...
global $post;
$base                  = 'single-content';
$related_content_query = Queries::related_content( 'post', get_the_category( $post->ID )[0]->term_id ?? '', $post->ID ?? '', 3 );
?>
	<section class="<?php echo esc_attr( $base ); ?>">
		<div class="<?php echo esc_attr( $base ); ?>__inner">
			...
			<div class="<?php echo esc_attr( $base ); ?>__grid">
				<svg class="<?php echo esc_attr( $base ); ?>__image-cutoff" width="150" height="149"
					 viewBox="0 0 150 149" fill="none" xmlns="http://www.w3.org/2000/svg">
					<path d="M159 0L159 149L0 1.39002e-05L159 0Z" fill="#283741"/>
				</svg>
				<div>
					<article class="<?php echo esc_attr( $base ); ?>__article">
						<h5 class="<?php echo esc_attr( $base ); ?>__headline"><?php the_title(); ?></h5>
						<p class="<?php echo esc_attr( $base ); ?>__author">Written by: <?php the_author(); ?></p>
						<p class="<?php echo esc_attr( $base ); ?>__post-date"><?php echo get_the_date( 'dS F Y' ); ?></p>
					</article>
				</div>
				<div>
					<article class="<?php echo esc_attr( $base ); ?>__article">
...
<?php
get_template_part(
	'template-parts/related-content',
	null,
	[
		'data' => [
