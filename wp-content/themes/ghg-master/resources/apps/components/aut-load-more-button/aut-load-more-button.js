import './aut-load-more-button.scss';
import { useContext, useEffect, useState } from 'react';
import seriesContext from '../../context/seriesContext';

const AutLoadMoreButton = (props) => {
	const { action, button_text } = props;
	const className = 'aut-load-more-button';
	...
	useEffect(() => {
		if (seriesGlobal.additional_data) {
			...
			if (seriesGlobal.no_results === true) {
				console.log(seriesGlobal.no_results);
				setShowButton(false);
			}
		}
	}, [seriesGlobal]);
		...
		try {
			const response = await fetch(ajax_params.ajaxurl, {
				...
				body: new URLSearchParams({
					...
					sort_by: seriesGlobal.additional_data.sort_by,
				}),
			});
...
export default AutLoadMoreButton;
